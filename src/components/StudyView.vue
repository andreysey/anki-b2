...
<template>
  <div class="flex flex-col items-center self-center gap-4 max-w-[580px] w-full px-1 sm:px-0 py-0 animate-in fade-in duration-500">
    ...
    <div class="flex items-center gap-2.5 w-full">
      ...
      />
      <div class="flex-1 text-center py-2 px-3 rounded-xl bg-white/95 dark:bg-slate-900/90 border border-slate-200/90 dark:border-white/10 flex flex-col items-center justify-center gap-0.5 text-xs font-semibold text-slate-700 dark:text-slate-300 shadow-2xs">
        <div>{{ currentStudyIndex + 1 }} / {{ vocabulary.length }}</div>
        <div class="text-[10px] text-slate-500 dark:text-slate-400 font-normal hidden sm:block">Space to flip &bull; Arrows to navigate</div>
      </div>
      <Button 
    ...
    <div class="flex flex-wrap items-center justify-center gap-x-5 gap-y-1.5 py-2 px-4 rounded-xl bg-white/80 dark:bg-black/30 border border-slate-200/80 dark:border-white/5 text-[11px] text-slate-500 dark:text-slate-400 w-full shadow-2xs">
