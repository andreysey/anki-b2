    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
      ...
      >
        ...
        <div class="flex justify-between items-center w-full mb-2.5">
          <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold bg-blue-50 text-blue-600 border border-blue-200 dark:bg-blue-500/15 dark:text-blue-400 dark:border-blue-500/25">
            {{ item.level }}
          </span>
          <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-medium bg-slate-100/90 text-slate-700 border border-slate-200/80 dark:bg-white/10 dark:text-slate-300 dark:border-white/10 max-w-[170px] truncate" :title="getThemaLabel(item.thema)">
        ...
        <div class="space-y-2.5">
          <div class="flex justify-between items-start gap-2.5">
            <div class="text-base sm:text-lg font-bold text-slate-900 dark:text-white leading-snug select-text" v-html="sanitizeHtml(item.german)"></div>
          ...
          <template v-if="item.example">
            <Divider class="!my-2" />
            <div class="flex items-center justify-between gap-2 p-2 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200/60 dark:border-white/5">
              <div class="italic text-slate-700 dark:text-slate-400 text-[11px] leading-relaxed [&_strong]:text-primary-600 [&_strong]:dark:text-primary-400 [&_b]:text-primary-600 [&_b]:dark:text-primary-400 select-text" v-html="sanitizeHtml(item.example)"></div>
              <Button 
                icon="pi pi-volume-up" 
                rounded 
                text 
